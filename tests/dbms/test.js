...
function renderNav() {
  ...
  state.questions.forEach((q, i) => {
    ...
    b.textContent = i + 1;
    if (i === state.current) b.classList.add("nav-active");
    else if (a?.checked === true) b.classList.add(a.correct ? "nav-good" : "nav-bad");
    else if (a?.choice !== undefined) b.classList.add("nav-selected");
    else b.classList.add("nav-empty");
...
function renderQuestion() {
  ...
  const a = state.answers[q.id];
  if (!a.qText) a.qText = q.question[0];
  if (!a.order) a.order = shuffle(q.options.map((_, i) => i));
    ...
    <div class="card">
      ...
      <div class="options">
        ${a.order.map((opt, i) => {
          ...
          } else if (a.choice === i) cls = "selected";
          return `
...
function finish() {
  ...
  state.questions.forEach(q => {
    const a = state.answers[q.id];
    if (!a || a.choice === undefined) detailed[q.id] = { skipped: true };
    else {
      if (a.correct) score++;
